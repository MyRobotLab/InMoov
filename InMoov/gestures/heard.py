def heard(data):
  global walkingThread

  print("Heard Called :" + str(data))

  if (data == "hi there"):
    lookaroundyou(data)

  if (data == "are you alright"):
    x = (random.randint(1, 2))
    inMoov.setHeadVelocity(43.0,-1,43.0,43.0,-1)
    inMoov.moveHead(90,60,90,180,65)
    sleep(1)
    inMoov.moveHead(90,120,90,180,65)
    sleep(1)
    inMoov.moveHead(90,60,90,180,65)
    sleep(1)
    inMoov.moveHead(90,120,90,180,65)
    sleep(1)
    inMoov.moveHead(90,90,90,180,65)
    sleep(1)
    if x == 1:
      inMoov.mouth.speak("i am very, super, mega bored")
      inMoov.moveArm("left",85,93,42,16)
      inMoov.moveArm("right",87,93,37,18)
      inMoov.moveHand("left",124,82,65,81,41,143)
      inMoov.moveHand("right",59,53,89,61,36,21)
      inMoov.moveTorso(90,90,90)
      sleep(0.2)
      inMoov.setHeadVelocity(-1,-1,-1,-1,-1)
      inMoov.moveHead(90,90,90,90,65)
      sleep(1)
      relax()
    if x == 2:
      inMoov.mouth.speak("I feel like a machine, doing the same thing over and over")
      inMoov.moveArm("left",85,93,42,16)
      inMoov.moveArm("right",87,93,37,18)
      inMoov.moveHand("left",124,82,65,81,41,143)
      inMoov.moveHand("right",59,53,89,61,36,21)
      inMoov.moveTorso(90,90,90)
      sleep(0.2)
      inMoov.setHeadVelocity(-1,-1,-1,-1,-1)
      inMoov.moveHead(90,90,90,90,65)
      sleep(1)
      relax()

  if (data == "you did great"):
    perfect()

  if (data == "show your back"):
    inMoov.mouth.speak("you know I don't like to show my back because it is not finished")

  if (data == "not true it's finished"):
    x = (random.randint(1, 3))
    inMoov.mouth.speak("oh")
    forwardServo.attach()
    directionServo.moveTo(45)
    forwardServo.moveTo(60)
    sleep(0.2)
    inMoov.mouth.speak("really")
    forwardServo.disable()
    fullspeed()
    inMoov.moveHead(16,11)
    inMoov.moveArm("left",60,67,67,40)
    inMoov.moveArm("right",5,116,10,28)
    inMoov.moveHand("left",143,69,48,2,2,23)
    inMoov.moveHand("right",89,60,78,43,68,163)
    inMoov.moveTorso(161,62,92)
    sleep(3)
    rest()
    sleep(1)
    relax()
    if x == 1:
      inMoov.mouth.speak("okay then, as you please")
      inMoov.moveHead(90,90)
    if x == 2:
      inMoov.mouth.speak("oh, yes I forgot")
      inMoov.moveHead(90,90)
    if x == 3:
      inMoov.mouth.speak("oh, I will turn around")
      inMoov.moveHead(90,90)

  if (data == "it's an object"):
    inMoov.mouth.speak("definition of an object, anything that is visible or tangible and relatively stable in form. ")


  if (data == "i know but show it anyway"):
    x = (random.randint(1, 3))
    if x == 1:
      inMoov.mouth.speak("as you please")
    if x == 2:
      inMoov.mouth.speak("i don't like that")
    if x == 3:
      inMoov.mouth.speak("alright")
    unhappy()

  if (data == "sorry i forgot"):
    x = (random.randint(1, 2))
    if x == 1:
      inMoov.mouth.speak("that's alright")
    if x == 2:
      inMoov.mouth.speak("you forget all the time")

  if (data == "it's okay"):
    inMoov.mouth.speak("good")


  if (data == "very good, thank you"):
    inMoov.mouth.speak("okay, good")

  if (data == "look at the people"):
    inMoov.setHeadVelocity(36.0, 36.0)
    for y in range(0, 10):
      x = (random.randint(1, 5))
      if x == 1:
        fullspeed()
        inMoov.head.neck.moveTo(90)
        eyeslooking(data)
        sleep(2)
        trackHumans()
        sleep(10)
        stopTracking()
      if x == 2:
        fullspeed()
        inMoov.head.rothead.moveTo(80)
        eyeslooking(data)
        sleep(2)
        trackHumans()
        sleep(10)
        stopTracking()
      if x == 3:
        fullspeed()
        headdown()
        sleep(1)
        trackHumans()
        sleep(10)
        stopTracking()
      if x == 4:
        fullspeed()
        lookrightside()
        sleep(2)
        trackHumans()
        sleep(10)
        stopTracking()
      if x == 5:
        fullspeed()
        lookleftside()
        sleep(2)
        trackHumans()
        sleep(10)
        stopTracking()
    sleep(1)
    lookinmiddle()
    sleep(3)
    inMoov.mouth.speak("nice to meet you all")

  if (data == "take a look around"):
    lookaroundyou()

  if (data == "good morning"):
    inMoov.mouth.speak("good morning")
    x = (random.randint(1, 4))
    if x == 1:
      inMoov.mouth.speak("i hope you had a good night sleep")
    if x == 2:
      inMoov.mouth.speak("nice to see you again")
    if x == 3:
      inMoov.mouth.speak("this is going to be a good day")

  if (data == "very good"):
    inMoov.mouth.speak("thanks")

  if (data =="italian hello"):
    italianhello()

  #if (data =="finnish hello"):
    #finnishhello()

  if (data =="do you like to eat indian food "):
    fullspeed()
    inMoov.setHeadVelocity(43.0, 36, 50, 50, -1)
    inMoov.moveHead(60,40,7,85,52)
    sleep(1)
    inMoov.moveHead(80,40,7,85,52)
    sleep(2)
    inMoov.setHeadVelocity(0.92, 36, 50, 50, -1)
    inMoov.moveHead(100,40,7,85,52)
    sleep(0.4)
    inMoov.moveArm("left",85,106,25,18)
    inMoov.moveArm("right",87,107,32,18)
    inMoov.moveHand("left",110,62,56,88,81,145)
    inMoov.moveHand("right",78,88,101,95,81,27)
    inMoov.moveTorso(90,90,90)
    inMoov.moveHead(80,40,7,85,52)
    inMoov.mouth.speakBlocking("yes, i want some paneer tikka")
    sleep(1)
    inMoov.moveHead(60,90,80,90,52)
    sleep(0.8)
    relax()

  if (data =="do you speak hindi"):
    inMoov.mouth.speak("yes, i can speak any language")
    inMoov.moveHead(116,80)
    inMoov.moveArm("left",85,93,42,16)
    inMoov.moveArm("right",87,93,37,18)
    inMoov.moveHand("left",124,82,65,81,41,143)
    inMoov.moveHand("right",59,53,89,61,36,21)
    inMoov.moveTorso(90,90,90)
    sleep(0.2)
    sleep(1)
    relax()

  if (data == "where are you from"):
    phonehome()

  if (data == "i know"):
    x = (random.randint(1, 3))
    if x == 1:
      inMoov.mouth.speak("yes, me too")
    if x == 2:
      inMoov.mouth.speak("I do too")
    if x == 3:
      inMoov.mouth.speak("sorry about that")

  if (data == "sorry"):
    x = (random.randint(1, 3))
    if x == 1:
      inMoov.mouth.speak("no problems")
    if x == 2:
      inMoov.mouth.speak("it doesn't matter")
    if x == 3:
      inMoov.mouth.speak("it's okay")

  if (data == "nice"):
    x = (random.randint(1, 3))
    if x == 1:
      inMoov.mouth.speak("I know")
    if x == 2:
      inMoov.mouth.speak("yes, indeed")
    if x == 3:
      inMoov.mouth.speak("you are damn right")

  if (data == "hello"):
    hello()
    relax()

  if (data == "bye bye"):
    inMoov.mouth.speak("see you soon")
    global helvar
    helvar = 1
    x = (random.randint(1, 2))
    if x == 1:
      inMoov.mouth.speak("i'm looking forward to see you again")
    if x == 2:
      inMoov.mouth.speak("goodbye")

  if (data == "thank you"):
    x = (random.randint(1, 3))
    if x == 1:
      inMoov.mouth.speak("you are welcome")
    if x == 2:
      inMoov.mouth.speak("my pleasure")
    if x == 3:
      inMoov.mouth.speak("it's okay")

  if (data == "thanks"):
    x = (random.randint(1, 2))
    if x == 1:
      inMoov.mouth.speak("it's okay")
    if x == 2:
      inMoov.mouth.speak("sure")

  if (data == "go forward"):
      #forwardServo.moveTo(60)
      # only start back up if we haven't already started.
      if not walkingThread.running:
        walkingThread.start()

  if (data == "walk gesture"):
      for _ in itertools.repeat(None):
        fullspeed()
        relax()
        sleep(1)
        rest()
      #inMoov.setHandVelocity("left", -1, -1, -1, -1, -1, -1)
      #inMoov.setHandVelocity("right", -1, -1, -1, -1, -1, -1)
      #inMoov.setArmVelocity("right", 59, 59, 59, 43.0)
      #inMoov.setArmVelocity("left", 59, 59, 59, 43.0)
      #inMoov.setHeadVelocity(31.0, 31.0)
      #inMoov.moveHead(70,79,85,85,65)
      #inMoov.moveArm("left",5,90,10,10)
      #inMoov.moveArm("right",15,90,40,10)
      #inMoov.moveHand("left",92,33,37,71,66,10)
      #inMoov.moveHand("right",81,66,82,60,105,100)
      #inMoov.moveTorso(75,97,90)
      #sleep(1)
      #inMoov.moveHead(79,100,85,85,65)
      #inMoov.moveArm("left",15,84,43,15)
      #inMoov.moveArm("right",5,82,10,20)
      #inMoov.moveHand("left",92,33,37,71,66,50)
      #inMoov.moveHand("right",81,66,82,60,105,150)
      #inMoov.moveTorso(124,83,90)
        sleep(1)

  if (data == "go backwards"):
      forwardServo.moveTo(110)
      relax()

  if (data == "watch out and stop"):
      walkingThread.running = False
      forwardServo.moveTo(93)
      # join the thread / let it stop
      walkingThread.join()
      # create a new one
      walkingThread = WalkingThread(i01, forwardServo)
      try:
        relax()
      except:
        print "It's difficult to relax..."

  if (data == "walking off"):
      walkingThread.running = False
      # join the thread / let it stop
      walkingThread.join()
      # create a new one
      walkingThread = WalkingThread(i01, forwardServo)
      try:
        relax()
      except:
        print "It's difficult to relax..."



  if (data == "to the left"):
      directionServo.moveTo(135)
      inMoov.setHeadVelocity(31.0, 31.0)
      inMoov.moveHead(70,124)

  if (data == "to the right"):
      directionServo.moveTo(45)
      inMoov.setHeadVelocity(31.0, 31.0)
      inMoov.moveHead(70,63)

  if (data == "go straight"):
      directionServo.moveTo(83)
      inMoov.setHeadVelocity(31.0, 31.0)
      inMoov.moveHead(70,80)

  if (data == "disconnect wheel"):
      directionServo.disable()
      forwardServo.disable()
  if (data == "attach wheel"):
      directionServo.attach()
      forwardServo.attach()

  if (data == "how do you do"):
    if helvar <= 2:
      inMoov.mouth.speak("I'm fine thank you")
      global helvar
      helvar += 1
    elif helvar == 3:
      inMoov.mouth.speak("you have already said that at least twice")
      inMoov.moveArm("left",43,88,22,10)
      inMoov.moveArm("right",20,90,30,10)
      inMoov.moveHand("left",0,0,0,0,0,119)
      inMoov.moveHand("right",0,0,0,0,0,119)
      sleep(2)
      relax()
      global helvar
      helvar += 1
    elif helvar == 4:
      inMoov.mouth.speak("what is your problem stop saying how do you do all the time")
      inMoov.moveArm("left",30,83,22,10)
      inMoov.moveArm("right",40,85,30,10)
      inMoov.moveHand("left",130,180,180,180,180,119)
      inMoov.moveHand("right",130,180,180,180,180,119)
      sleep(2)
      relax()
      global helvar
      helvar += 1
    elif helvar == 5:
      inMoov.mouth.speak("i will ignore you if you say how do you do one more time")
      unhappy()
      sleep(4)
      relax()
      global helvar
      helvar += 1

  if (data == "i love you"):
    inMoov.mouth.speak("i love you too")
    inMoov.moveHead(116,80)
    inMoov.moveArm("left",85,93,42,16)
    inMoov.moveArm("right",87,93,37,18)
    inMoov.moveHand("left",124,82,65,81,41,143)
    inMoov.moveHand("right",59,53,89,61,36,21)
    inMoov.moveTorso(90,90,90)
    sleep(0.2)
    sleep(1)
    relax()

  if (data == "what is the weather"):
    global weathervar
    if weathervar <= 2:
      inMoov.mouth.speak("I have no idea, I am not connected to internet")
      weathervar += 1
    elif weathervar == 3:
      inMoov.mouth.speak("Sorry, I told you, I am not connected to internet")
      inMoov.moveArm("left",43,88,22,10)
      inMoov.moveArm("right",20,90,30,10)
      inMoov.moveHand("left",0,0,0,0,0,119)
      inMoov.moveHand("right",0,0,0,0,0,119)
      sleep(2)
      relax()
      weathervar += 1
    elif weathervar == 4:
      inMoov.mouth.speak("Gael, you are annoying, stop asking me the weather")
      inMoov.moveArm("left",30,83,22,10)
      inMoov.moveArm("right",40,85,30,10)
      inMoov.moveHand("left",130,180,180,180,180,119)
      inMoov.moveHand("right",130,180,180,180,180,119)
      sleep(2)
      relax()
      weathervar += 1
    elif weathervar == 5:
      inMoov.setHeadVelocity(59, 59, 50, 50, -1)
      inMoov.moveHead(80,66)
      sleep(1)
      inMoov.setHeadVelocity(59, 59, 50, 50, -1)
      inMoov.moveHead(80,110)
      sleep(1)
      inMoov.setHeadVelocity(59, 59, 50, 50, -1)
      inMoov.moveHead(80,66)
      sleep(1)
      inMoov.setHeadVelocity(59, 59, 50, 50, -1)
      inMoov.moveHead(80,110)
      sleep(1)
      inMoov.setHeadVelocity(59, 59, 50, 50, -1)
      inMoov.moveHead(80,66)
      sleep(1)
      inMoov.mouth.speak("Well, well, Humans are worst than robots, they never learn")
      fullspeed()
      inMoov.moveArm("left",85,106,25,18)
      inMoov.moveArm("right",87,107,32,18)
      inMoov.moveHand("left",110,62,56,88,81,145)
      inMoov.moveHand("right",78,88,101,95,81,27)
      inMoov.moveTorso(90,90,90)
      sleep(4)
      relax()
      weathervar += 1





