def whataboutstarwars():
  inMoov.setNeopixelAnimation("Ironman", 255, 255, 255, 1)
  sleep(3)
  inMoov.stopNeopixelAnimation()
  inMoov.startedGesture()
  x = (random.randint(1, 3))
  if x == 1:
      fullspeed()
      inMoov.moveHead(130,149,87,80,100)
      AudioPlayer.playFile(RuningFolder+'/system/sounds/R2D2.mp3')
      #inMoov.mouth.speak("R2D2")
      sleep(1)
      inMoov.moveHead(155,31,87,80,100)
      sleep(1)
      inMoov.moveHead(130,31,87,80,100)
      sleep(1)
      inMoov.moveHead(90,90,87,80,100)
      sleep(0.5)
      inMoov.moveHead(90,90,87,80,0)
      sleep(1)
      relax()
  if x == 2:
      fullspeed()
      #inMoov.mouth.speak("Hello sir, I am C3po unicyborg relations")
      AudioPlayer.playFile(RuningFolder+'/system/sounds/Hello sir, I am C3po unicyborg relations.mp3')
      inMoov.moveHead(138,80)
      inMoov.moveArm("left",79,42,23,41)
      inMoov.moveArm("right",71,40,14,39)
      inMoov.moveHand("left",180,180,180,180,180,47)
      inMoov.moveHand("right",99,130,152,154,145,180)
      inMoov.moveTorso(90,90,90)
      sleep(1)
      inMoov.moveHead(116,80)
      inMoov.moveArm("left",85,93,42,16)
      inMoov.moveArm("right",87,93,37,18)
      inMoov.moveHand("left",124,82,65,81,41,143)
      inMoov.moveHand("right",59,53,89,61,36,21)
      inMoov.moveTorso(90,90,90)
      sleep(1)
      relax()
  if x == 3:
      inMoov.setHandVelocity("left", 43.0, 43.0, 43.0, 43.0, 43.0, -1)
      inMoov.setHandVelocity("right", -1, 43.0, -1, -1, -1, -1)
      inMoov.setArmVelocity("left", -1.0, -1.0, -1.0, -1.0)
      inMoov.setArmVelocity("right", 50, -1, -1, -1)
      inMoov.setHeadVelocity(-1, 50)
      inMoov.setTorsoVelocity(-1, -1, -1)
      inMoov.moveHead(80,86)
      inMoov.moveArm("left",5,94,30,10)
      inMoov.moveArm("right",7,74,50,10)
      inMoov.moveHand("left",180,180,180,180,180,90)
      inMoov.moveHand("right",180,2,175,160,165,180)
      inMoov.moveTorso(90,90,90)
      #inMoov.mouth.speak("mmmmmmh, from the dark side you are")
      AudioPlayer.playFile(RuningFolder+'/system/sounds/mmmmmmh, from the dark side you are.mp3')
      sleep(4.5)
  inMoov.finishedGesture()
  relax()