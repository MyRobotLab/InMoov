def translateTextFR(text,language):
  ...
  try:
    ...
    inmoovFrench.getResponse("AZURE_ERROR_2 "+language)
  print RealLang
  
  try:
    AzureTranslator.detectLanguage(text)
  except:
    inmoovFrench.getResponse("AZURE_ERROR_1")
    RealLang="0"
  ...
  if RealLang!="0":
    ...
    if 'Cannot find an active Azure Market Place' in t_text:
      inmoovFrench.getResponse("AZURE_ERROR_3")
    else:
